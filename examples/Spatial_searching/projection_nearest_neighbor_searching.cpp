...
bool same_projection(const std::list<Point_d> &l1, const std::list<Point_d>& l2) { 
  typedef typename ProjectionTraits::Cartesian_const_iterator_d iterator_type;
  typename ProjectionTraits::Construct_cartesian_const_iterator_d construct_it =
     ProjectionTraits().construct_cartesian_const_iterator_d_object(); 
  if(l1.size() != l2.size())
    return false;
  std::list<Point_d>::const_iterator it1;
  std::list<Point_d>::const_iterator it2;
  for(it1 = l1.begin(), it2 = l2.begin(); it1 != l1.end(); it1++, it2++) {
    iterator_type coord_it1 = construct_it(*it1), coord_end1 = construct_it(*it1, 1);
    iterator_type coord_it2 = construct_it(*it2), coord_end2 = construct_it(*it2, 1);
...
void create_points(std::list<Point_d>& ret, int x_idx, int y_idx, int size) {
  int i = 0;
